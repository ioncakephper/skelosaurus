...
const {render_template} = require('hbsr');

/**
 * Checks if a given pattern is valid for use with globSync.
 *
 * The function returns false if the pattern is null, not a string, contains the string '<script>', or is an empty string.
 * It also checks if the pattern only contains valid globbing characters (letters, numbers, and *, ?, [, ], -, _, ., ^, $, +, @, !).
 *
...
function isValidPattern(pattern) {
    ...
    // Matches typical glob patterns accepted by globSync
...
/**
 * Normalize a single item in a sidebar configuration.
 *
 * @param {string|object} item - The item to normalize.
 * @returns {object} The normalized item.
 * @throws Will throw an error if the item is not a valid string or object.
 *
 * If the item is a string, it will be converted to an object with a single property named "label" with the value of the string.
 *
 * If the item is an object, it must have a "label" property with a non-empty string value. If the object has an "items" or "headings" property, it must be an array.
 *
 * If the item is invalid, it will throw an error.
 */
...
function findDuplicatedSidebars(files) {
    ...
    if (Object.keys(duplicatedSidebarLabels).length > 0) {
        console.error(`Duplicated sidebar labels: ${Object.keys(duplicatedSidebarLabels).sort().join(', ')}`)
    }
...
function getSidebars(file) {
    try {
        ...
        const { sidebars = [], ...rest } = parsedContent;
        return {
            ...rest, sidebars: sidebars.map(normalizeItem) || []
        }
 ...
 * @throws Will throw an error if `files` is not an array, or if the schema file cannot be found.
 */
function validateFiles(files, options) {
    ...
    if (Object.keys(invalidFiles).length > 0) {
        console.error(`Invalid files: ${Object.keys(invalidFiles).sort().join(', ')}`);
    }
...
/**
 * Builds sidebar items based on sidebar items and options.
 *
 * Iterates through `sidebarItems` and calls either `buildCategory`, `buildLink`, or `buildTopic` based on
 * the presence and values of `items` and `href` properties.
 *
 * @param {Array} sidebarItems - An array of sidebar item objects.
 * @param {object} options - Options to pass to `buildCategory`, `buildLink`, and `buildTopic`.
 * @returns {Array} An array of sidebar items after being processed.
 */
...
/**
 * Builds a sidebar item for a topic.
 *
 * Given a sidebar item, builds a topic sidebar item by joining the parent path with the topic path
 * and using the topic id as the label. If the topic id is not provided, uses the topic label.
 *
 * @param {object} sidebarItem - A sidebar item object.
 * @param {object} options - Options to pass to buildTopic.
 * @returns {object} A sidebar item object with a type of 'doc'.
 */
function buildTopic(sidebarItem, options) {
    ...
    const itemFullPath = [topicPath, slugify(itemId)].join('/');
    const cleanedItemFullPath = itemFullPath.replace(/^(\.\/)+/, '').replace(/^\.+|\.+$/g, '');
    try {
        createTopicDocument(sidebarItem, cleanedItemFullPath, options);
        return cleanedItemFullPath;
    } catch (error) {
        console.error(`Error creating topic document: ${error.message}`);
        return null;
    }

    return {
        type: 'doc',
        id: cleanedItemFullPath,
        label: sidebarItem.label || itemId,
    };
}

/**
 * Creates a topic document in Markdown.
 *
 * Creates a topic document in Markdown by joining the parent path with the topic path
 * and using the topic id as the label. If the topic id is not provided, uses the topic label.
 *
 * @param {object} sidebarItem - A sidebar item object representing a topic.
 *                              It is expected to have at least a `label` property.
 * @param {string} itemFullPath - The full path to the topic document.
 * @param {object} options - Options passed to createTopicDocument.
 */
function createTopicDocument(sidebarItem, itemFullPath, options) {
    const headings = buildHeaders(sidebarItem.headings, 2);
    const { id, slug, title, brief, summary, note } = extractProperties(sidebarItem);

    function extractProperties(sidebarItem) {
        return {
            id: sidebarItem.id,
            slug: sidebarItem.slug,
            title: sidebarItem.title,
            brief: sidebarItem.brief,
            summary: sidebarItem.summary,
            note: sidebarItem.note,
        };
    }
    ...
    const content = render_template(path.join(options.templates, 'topic-template.hbs'), templateVariables);
    if (!itemFullPath.endsWith('.md') && !itemFullPath.endsWith('.markdown')) {
        itemFullPath = `${itemFullPath}.md`;
    }

    // use a stream to write the file so we don't have to load the entire file into memory
    const documentFilename = path.join(options.docs, itemFullPath);
    const cleanedDocumentFilename = documentFilename.replace(/^(\.\/)+/, '').replace(/^\.+|\.+$/g, '');
    const writer = fs.createWriteStream(cleanedDocumentFilename);
...
/**
 * Builds headings from a flat array of headings.
 *
 * Given a flat array of headings, builds headings by joining the headings
 * with newlines and prefixing each heading with the appropriate number of
 * `#` characters. If the heading is an object, it is assumed to be a heading
 * with children and is recursively processed. If the heading is a string, it
 * is processed as a heading. If the heading is not a string or an object with
 * an `items` property, it is returned as is.
 *
 * @param {Array.<string|object>} headings - A flat array of headings.
 * @param {number} [level=2] - The level of the headings.
 * @returns {string} The headings as a string, joined by newlines.
 */
function buildHeaders(headings, level = 2) {
    return headings.flatMap(item => {
        if (typeof item === 'object') {
            return buildHeaders(item.items || [], level + 1);
        }
...
/**
 * Builds a sidebar item for a category.
 *
 * Constructs a category sidebar item by using the category id, slug, or label
 * as the label, and recursively processes the items within the category.
 *
 * @param {object} sidebarItem - A sidebar item object representing a category.
 * @param {object} options - Options to pass to `buildSidebar`.
 * @returns {object} A sidebar item object with a type of 'category'.
 */
...
/**
 * Builds a sidebar item for a link.
 *
 * Given a sidebar item, constructs a link sidebar item using the provided label or href as the label and sets the href property.
 *
 * @param {object} sidebarItem - A sidebar item object representing a link.
 * @param {object} options - Options to pass to buildLink.
 * @returns {object} A sidebar item object with a type of 'link'.
 * @throws Will throw an error if the sidebar item is not a valid object.
 * @throws Will throw an error if the sidebar item does not have a label or href property.
 * @throws Will throw an error if the sidebar item href property is not a string.
 */
function buildLink(sidebarItem, options) {
    ...
    if (typeof sidebarItem !== 'object') {
        throw new Error(`Invalid sidebar item: ${sidebarItem}`);
    }

    if (sidebarItem.href === null || sidebarItem.href === undefined || typeof sidebarItem.href !== 'string' || sidebarItem.href.trim() === '') {
        throw new Error(`Invalid sidebar item href: ${sidebarItem.href}`);
    }
...
// TODO: Double-check sorted export 
