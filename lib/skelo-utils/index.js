...
function buildCategory(item, options = {}) {
    ...
    sendMessage(options.verbose, 'create-mypackage', 'info', `Building category ${item.label}`);
    return {
        label: item.label,
        type: 'category',
...
function createTopicDocument(item, topicBasename, options = {}) {
    ...
    saveDocument(`${options.docs}/${topicBasename}.md`, topicContent);
    sendMessage(options.verbose, options.name, 'info', `Created ${topicBasename}.md`);
